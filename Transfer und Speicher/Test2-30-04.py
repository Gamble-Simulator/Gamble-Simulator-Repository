...
symbole = [(0, "red"), (1, "green"), (2, "blue"), (3, "yellow"), (4, "white"), (5, "lol"), (6, "loli")]
anzSymbole = 7
walzen = ["", "", "", "", ""]
maxSchritte = 5
drehen = True
...
def checkWin(all):
    for i in range(5):
        if all[i].symbol[] == all[i+1[1]].symbol:
            print("win")
"""
...
class Walze():
    ...
    def felderAdden(self):
        ...
        self.__felder.append(Feld(1, symbole[random.randint(0, anzSymbole-1)], 1, points[self.__spalte-1]))
        self.__felder.append(Feld(2, symbole[random.randint(0, anzSymbole-1)], 1, points[self.__spalte+4]))
        self.__felder.append(Feld(3, symbole[random.randint(0, anzSymbole-1)], 1, points[self.__spalte+9]))
    ...
    def animation():
        ...
        if drehSchritte < maxSchritte:
            ...
            drehen = False
            drehButton.place(x=15*WIDTH // 64 + WIDTH//200, y=HEIGHT//3 + HEIGHT//2 + HEIGHT//80, width=WIDTH//12, height=HEIGHT//8)
...
class Feld(Walze):
    ...
    def neues_symbol(self):
        self.__symbol = symbole[random.randint(0, anzSymbole-1)]
...
message_id = canvas.create_text(
    3*WIDTH // 4, (5/6)*HEIGHT + HEIGHT//100,
    text="DREHEN drücken",
    fill="white",
    font=("Arial", 20, "bold")
)
...
#Button erstellen
drehButton = tk.Button(text = "Drehen", font=("Arial", 20, "bold"), command = Walze.animation, bg = "red")
drehButton.place(x=15*WIDTH // 64 + WIDTH//200, y=HEIGHT//3 + HEIGHT//2 + HEIGHT//80, width=WIDTH//12, height=HEIGHT//8)
